...
function App() {
  ...
  return (<>
    <div className="loading-bars">
      <div className="loading-correct">
        <div className="progress">
          <div className="progress-bar bg-success" role="progressbar" style={{width: `${100*(score/questions.length)}%`}} aria-valuenow="25" aria-valuemin="0" aria-valuemax="100"></div>
        </div>
        <span className="text-correct">{score} / { questions.length}</span>
      </div>



      <div className="counter">{100*(score/questions.length)}</div>
      <div className="loading-wrong">
        <div className="progress">
          <div className="progress-bar bg-danger" role="progressbar" style={{width: `${100*((currentQuestion - score)/questions.length)}%`}}aria-valuenow="100" aria-valuemin="0" aria-valuemax="100"></div>
        </div>
          <span className="text-wrong">{currentQuestion - score} / { questions.length}</span>
      </div>
    </div>
    <div className="app">
      {showScore ? (
